...
pub fn envia_mensagem_whatsapp_sandbox(mensagem: String) {
    ...
    let sandbox_number = env::var("TWILIO_PHONE").expect("Defina a variável TWILIO_PHONE");
    let client_number =
        env::var("TWILIO_CLIENT_NUMBER").expect("Defina a variável TWILIO_CLIENT_NUMBER");
...
pub fn envia_arquivo_whatsapp_sandbox(mensagem: String, nome_arquivo: &'static str) {
    ...
    tokio::spawn(async move {
        ...
        let (_, return_code) = bucket
            .put_object(&format!("/{}", nome_arquivo), &buffer)
            .await
            .unwrap();
