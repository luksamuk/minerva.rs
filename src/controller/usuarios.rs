...
pub fn encontra_usuario(conexao: &PgConnection, usr_login: String) -> Option<Usuario> {
    let usr_req = usuario.filter(login.eq(&usr_login))
        .load::<Usuario>(conexao)
        .expect("Erro ao carregar usuário");
    match usr_req.first() {
        None => None,
...
pub fn registra_usuario(conexao: &PgConnection, dados: &UsuarioRecv) -> Result<(i32, String), String> {
        ...
        .get_result::<Usuario>(conexao)
    {
        ...
        Err(e) => {
            if let diesel::result::Error::DatabaseError(_, _) = &e {
                ...
                Err(String::from("Erro interno ao cadastrar usuário. \
