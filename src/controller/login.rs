...
pub fn loga_usuario(conexao: &PgConnection, redis: &mut RedisConnection, dados: &LoginData) -> Resposta {
    // 1. Verifica se o usuário existe.
    let usuario =
        match usuarios::encontra_usuario(conexao, dados.login.to_owned()) {
            Some(usuario) => usuario,
            None => return Resposta::NaoEncontrado(String::from(
                "{ \"mensagem\": \"Usuário não encontrado\" }")),
        };
    
    // 2. Testa a senha do usuário.
    if !usuario.verifica_senha(dados.senha) {
        return Resposta::NaoAutorizado(String::from(
            "{ \"mensagem\": \"Senha incorreta\" }"));
    }
    
    // 3. Gera token JWT.
    let token = match jwt::cria_jwt(&dados.login) {
        Err(erro) => return Resposta::ErroInterno(format!(
            "{{ \"mensagem\": \"Erro ao gerar token JWT: {}\" }}", erro)),
        Ok(tok) => tok,
    };
    ...
    // 5. Retorna o token.
    Resposta::Ok(serde_json::to_string(&LoginResponse {
        id: usuario.id,
